...
function render() {
    ...
    var assignmentBandLine = bandLine()
        ...
        .pointStyleAccessor(function(v) {return v < 0.60 ? 'low' : v > 0.90 ? 'high' : 'normal'})
    ...
    ;(function renderDistributions(root) {
        ...
        renderGradeHistogram(root, compose(property('current'), property('grades')))

        ;(function renderAssignmentDistributions() {

            bind(root, 'lastAssignmentDistributions')
                .entered
                .attr({
                    transform: translateX(323)
                })

            renderGradeHistogram(root['lastAssignmentDistributions'], function(d) {return s.scoreToGrade(last(d.assignmentScores));})

            renderDistributionLegends(root['lastAssignmentDistributions'], lastAssignmentDistributionStyleSet)

        })()

    })(distributionsGroup.group)

    ;(function renderTsers(root) {

        var xScale = s.temporalScale
        var yScale1 = s.absentScale
        var yScale2 = s.absentScale

        bind(root, 'chart')
        bind(root['chart'], 'chartLine', 'g', function(d) {
            return [d.Absences, d.Tardies]
                .map(function(eventSet, i) {
                    var keptEventSet = eventSet.filter(function(event) {return keptStudentFilterFunction(event[0])})
                    var sparseValues = pairs(countBy(keptEventSet, property(1))).map(function(tuple) {return [new Date(tuple[0]), tuple[1]]})
                        .sort(tupleSorter)
                    var denseValues = []
                    var baseDateValue = xScale.domain()[0].valueOf()
                    var now = (new Date('2012-05-01')).valueOf()
                    for(var index = 0, day = baseDateValue; day <= xScale.domain()[1].valueOf();  index++, day += 24 * 60 * 60 * 1000) {
                    ...
                    return {
                        key: ['Absent', 'Tardy'][i],
                        value: sparseValues,
                        denseValue: denseValues
                    }
                })})
            .entered
            .attr('transform', translate(33, function(d) {return d.key === 'Absent' ? 0 : 40.5}))
        ...
        bind(root['chart']['chartLine'], 'xAxis1')
        root['chart']['chartLine']['xAxis1']
            .entered
            .each(function(d) {if(d.key === 'Absent') xAxis1(d3.select(this)); else xAxis2(d3.select(this))})
        ...
        var xAxisTop = d3.svg.axis()
            ...
            .tickPadding(1)
            //.tickFormat(function() {return ""})
        bind(root['chart']['chartLine'], 'xAxisTop')
            .entered
            .each(function(d) {if(d.key !== 'Absent') xAxisTop(d3.select(this))})
            .attr('transform', translateY(-930))
            .attr('opacity', 100)
        ...
        var yAxis2 = d3.svg.axis()
            ...
            .orient('left')
            .ticks(1)
        bind(root['chart']['chartLine'], 'yAxis')
        root['chart']['chartLine']['yAxis']
            .entered
            .each(function(d) {if(d.key === 'Absent') yAxis1(d3.select(this)); else yAxis2(d3.select(this))})

        bind(root['chart']['chartLine']['yAxis'], 'axisLegend', 'text')
            .entered
            .text(key)
            .attr({
                x: 4,
                y: 37,
                transform: function(d) {var scale = d.key === 'Absent' ? yScale1 : yScale2; return translateY(last(scale.range()))(d)}
            })

        bind(root['chart']['chartLine'], 'tserLine', 'path')
            .entered
            .attr({
                stroke: function(d) {return d.key === 'Absent' ? palette.magenta : 'grey'},
                'stroke-width': 1.75
            })
        ...
        bind(root['chart']['chartLine'], 'tserLine', 'path')
            ...
            .attr({
                d: function(d) {
                    return d3.svg.line()
                        .x(compose(xScale, property(0)))
                        .y(compose(d.key === 'Absent' ? yScale1 : yScale2, property(1)))
                        .defined(function(n) {return n[1] !== null})(d.denseValue)
                }
            })
    ...
    ;(function renderBenchmarks(root) {
        ...
        var xScale = s.scoreBandScale
        var yScale = s.studentRatioScale

        bind(root, 'chart')
            .attr('transform', translateX(33))
        bind(root['chart'], 'chartLine', 'g', property('contextScores'))

        var xAxis = d3.svg.axis()
            .scale(xScale)
            .orient('bottom')
            .tickFormat(function(d) {return d ? (10 * (d + 4) + 1) + '-' + 10 * (d + 5) + '%' : "<= 50%"})
        bind(root['chart'], 'xAxis')
        ...
        bind(root['chart'], 'yAxis')
        root['chart']['yAxis']
                        .call(yAxis)
            . selectAll('.tick')
            .attr('font-size', 10)

        bind(root['chart']['chartLine'], 'markedLine')
            .entered
            .call(benchmarkStyler())

        bind(root['chart']['chartLine']['markedLine'], 'line', 'path')
                        .attr({d: function(d) {
                var drawer = d3.svg.line()
                    .x(compose(xScale, property(0)))
                    .y(compose(yScale, property(1)))
                    .defined(always)

                return drawer(d.distribution.map(function(s, i) {return [i, s]}))
            }})

    })(benchmarkGroup.group.entered)

    ;(function renderMedians(root) {

        bind(root, 'tableColumn', 'g', property('contextScores'))
            .attr({
                transform: translateX(function (d, i) {
                    return (i + 1) * 110
                })
            })

        bind(root['tableColumn'], 'header', 'text')
            .attr({
                y: '0.5em',
        ...
        bind(root['tableColumn'], 'value', 'text')
            .attr({
                y: '0.5em'
            })
            .text(function (d) {
                return Math.round(100 * d.median) + '%'
            })

    })(medianGroup.group.entered)

    ;(function renderBehaviorCountTotals(root) {
        bind(root, 'behaviorCountMetrics', 'g', function(d) {
            var aggregate = function(key) {
                return d['Student Data'].map(property(key)).reduce(add)
            }
            return [{
                key: 0,
                pastReferralCount: aggregate('pastReferralCount'),
                currentReferralCount: aggregate('currentReferralCount'),
                pastDetentionCount: aggregate('pastDetentionCount'),
                currentDetentionCount: aggregate('currentDetentionCount')
            }]
        })
        renderBehaviorCounts(root['behaviorCountMetrics'])

        bind(root['behaviorCountMetrics'], 'groupLegends')
        bind(root['behaviorCountMetrics']['groupLegends'], 'legend1', 'text')
            .entered
            .attr('transform', translate(behaviorOffsetX + 10, 35))
            .text('Last item')
        bind(root['behaviorCountMetrics']['groupLegends'], 'legend2', 'text')
            .entered
            .attr('transform', translate(behaviorOffsetX + 10, 50))
            .text('This item')

    })(behaviorAggregateGroup.group.entered)
    ...
    ;(function renderAssignmentScoresAggregates(root) {
        ...
        bind(root['assignmentAggregateMetrics'], 'legendText', 'text')
            .entered
            .text('Average Scores')
            .attr({dx: 2, dy: 36})
